...
async function handleMessage(userId, message) {
  ...
  const text = message.body; // Already normalized in client.js
  const userName = message.userName; // Use dynamically extracted user name
  const session = sessions.get(userId) || { state: 'GREETING', data: {} };

  // Reset session if user says "start again"
  if (text === 'start again') {
    sessions.delete(userId);
    return `Session reset, ${userName}. Reply MENU to start again.`;
  }
  ...
  if (text === 'menu') {
    ...
    return getMenuMessage(userName);
  }

  // Cancel order flow if user says "cancel"
  if (text === 'cancel') {
    sessions.delete(userId);
    return `Order cancelled, ${userName}. Reply MENU to start again.`;
  }

  // Respond to 'hi'
  if (text === 'hi') {
    return `Welcome to SnackStop, ${userName} 👋\nReply MENU to see today\'s items.`;
  }
  ...
  switch (session.state) {
    ...
    case 'MENU':
      if (['1', '2', '3', '4'].includes(text)) {
        ...
        sessions.set(userId, session);
        return `You selected ${items[text].name} ₹${items[text].price}, ${userName}.\nHow many would you like? Reply with a number.`;
      }
      return `Invalid input, ${userName}. Reply with the number to order (e.g., 2).`;
    ...
    case 'QUANTITY':
      ...
      return `Invalid quantity, ${userName}. Please reply with a valid number.`;
    ...
    case 'CONFIRMATION':
      if (text === 'yes') {
        ...
        sessions.set(userId, session);
        return `Order confirmed 🎉\nYour Order ID is ${orderId}, ${userName}.\n\nWe'll deliver in 30 minutes 🚀\n\nReply STATUS anytime to check your order.`;
      } else if (text === 'no') {
        sessions.delete(userId);
        return `Order cancelled, ${userName}. Reply MENU to start again.`;
      }
      return `Invalid input, ${userName}. Reply YES to confirm or NO to cancel.`;

    case 'ORDER_CONFIRMED':
      if (text === 'status') {
        return `Order ${session.data.orderId}\n${session.data.item.name} x${session.data.quantity}\nStatus: Preparing 🟡`;
      }
      return `Invalid input, ${userName}. Reply STATUS to check your order.`;
...
function getMenuMessage(userName) {
  return `Here\'s what we have today, ${userName}:\n\n1. Masala Popcorn - ₹49\n2. Cold Coffee - ₹79\n3. Veg Sandwich - ₹99\n4. Brownie - ₹59\n\nReply with the number to order (e.g., 2).`;
}
